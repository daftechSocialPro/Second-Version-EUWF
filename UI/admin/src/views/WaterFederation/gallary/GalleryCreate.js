...
import { urlGallery } from 'src/endpoints'
import { customToast } from 'src/components/customToast'

function GalleryCreate({ setIsLodding }) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      await axios.
        post(`${urlGallery}?jwt=${jwt}`, formData)
        .then((res) => {
          setIsLodding(false)
          setTitle('')
          setImg('')
          customToast('Gallery Successfully created', 0)
          navigate('/gallery')
        })
        .catch((err) => {
          setIsLodding(false)
      ...
      <CCol xs={12}>
        <CCard className="mb-4">
          <CCardHeader style={{ backgroundColor: '#1e4356', color: '#fff' }}>
            <strong>Add </strong> <small>Gallery</small>
          </CCardHeader>
          <CCardBody>
